...
const Settings = () => {
    ...
    const { userRole, userData, setUserData, accountDelete, signOutUser } = useAuth();
    const { username } = useParams();
    const navigate = useNavigate();

    useEffect(() => {
        Countries.countries().then(result => {
            setCountriesList(result.success ? (result.data === undefined ? [] : result.data) : []);
            setLoading(false);
        });
        ...
        SettingsHandler.updateProfile(
            ...
            if (result.success) {
                ...
                setUserData(previousData => ({
                    ...
                    birthDate: birthDate,
                    country: country,
                    image: `${usernameProfile}.${temporaryImage ? temporaryImage.name.split(".").pop() : userData.image.split(".").pop()}`
                }));
                setTemporaryImage(null);

                navigate(userRole === "ROLE_USER" ? `/${usernameProfile}/settings` : "/settings");
            } else {
    ...
    if (userData && username !== userData.username && (userRole !== "ROLE_ADMIN" && userRole !== "ROLE_USER")) {
        return <Page404 />
    }
                        ...
                        <div className={"mb-4"}>
                            ...
                            <div className={"mb-3"}>
                                ...
                                <div className={"d-flex align-items-end"}>
                                    ...
                                    <img
                                        id={"formPhoto"}
                                        src={process.env.REACT_APP_PROFILE_IMAGES + userData.image}
                                        alt={"Profile"}
                                        name={"formPhoto"}
